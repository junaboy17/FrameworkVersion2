...
public class LookupsPageTest extends BaseHook{
	...
	public void DataTableTitleCheck(String Module) {
		...
		Source		.AdministratorNavigator()
					.clickLookups();
		Expected = "active";
		Actual = Source.SeleniumHelper().getAttribute(Source.AdministratorNavigator().LookupsTab, "class");
		SoftAssert.assertEquals(Actual, Expected);
	...
	public void SearchFunctionsCheck(String Module) {		
		...
		Source.DataTable().setSearch("Code"+ Keys.TAB);
		Source.SeleniumHelper().verifyAllText(Source.DataTable().allDataResult, "CODE", "Code");
